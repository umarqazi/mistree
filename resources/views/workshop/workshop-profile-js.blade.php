<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#services option:selected').each(function(){
            var service_rate    = "{{ old('service-rates.index') }}".replace("{index}", $(this).val());
            var service_time    = "{{ old('service-times.index') }}".replace("{index}", $(this).val());
                            ...
                            '<div class="row">' +
                                '<div class="col-md-6">' +
                                    '<div class="form-group">' +
                                        ...
                                        '<input type="text" class="form-control border-input" name="service-rates[' + $(this).val() +']" value="' + service_rate + '">' +
                                    '</div>' +
                                '</div>' +
                                '<div class="col-md-6">' +
                                    '<div class="form-group">' +
                                        '<label class="control-label">Service Time <span class="manadatory">*</span></label>' +
                                        '<input type="text" class="form-control border-input" name="service-times[' + $(this).val() +']" value="' + service_time + '">' +
                                    '</div>' +
                                '</div>' +
                            '</div>' +
                        '</div>' +
